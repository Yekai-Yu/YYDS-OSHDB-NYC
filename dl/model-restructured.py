...
def train_model(model, train_loader, val_loader, n_epochs, logfile):
    ...
    for epoch in range(n_epochs):
        ...
        log.write('Metric on validation dataset:\n')
        ...
        val_loss = 0       
        with torch.no_grad():
            val_loss = eval_model(epoch, model, val_loader, logfile, "validation")
...
def eval_model(epoch, model, test_loader, logfile, setstr):
    log = open(logfile, "a")
...
logfile = "runlog.txt"
r2_train = np.zeros((N_LABEL, N_EPOCH))
...
model.eval()
with torch.no_grad():
    eval_model(-1, model, test_loader, logfile, "test (last epoch)")
...
for i in range(N_LABEL):
    for j in range(N_EPOCH):
        log.write("{:.6f} ".format(r2_train[i][j]))
    log.write("\n")
log.close()